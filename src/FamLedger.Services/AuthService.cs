...
{
    public async Task<string> AuthenticateTelegramAsync(long id, string? firstName, string? username, string? photoUrl, long authDate, string hash, CancellationToken ct = default)
    {
        ...
        if (firstName is not null) fields["first_name"] = firstName;
        if (username is not null) fields["username"] = username;
        if (photoUrl is not null) fields["photo_url"] = photoUrl;
